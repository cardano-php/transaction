...
runs inside libsodium. The package decodes the point's ristretto255 encoding to its four Edwards candidates in
PHP, and keeps the one whose signature verifies. That decoding runs on the published half of the signature and
nothing else.

This needs PHP built against libsodium 1.0.18 or newer. Without GMP, reading a key takes about 190 milliseconds and
each signature about 20 milliseconds. A seed key signs in well under one millisecond.
...
Extended keys are checked against the tools that write them. Two throwaway keys under
`tests/fixtures/cardano-signer`, never funded, sign each committed message to cardano-signer's signature byte for
byte, and witness a mainnet body exactly as cardano-cli does. The RFC 8032 seeds signed through the extended path
reproduce the published signatures, and the ristretto255 decoder is checked against the RFC 9496 vectors under
`tests/fixtures/ristretto255`.
