...
| `Cardano\Transaction\Signing` | An Ed25519 or extended BIP32-Ed25519 key that lives for one call, and the signer that turns a body hash into a witness set. |
...
A key derived from a mnemonic down a CIP-1852 or CIP-1855 path is an extended BIP32-Ed25519 key, which is what
cardano-cli and cardano-signer write as `PaymentExtendedSigningKeyShelley_ed25519_bip32` or
`ExtendedSigningKeyShelley_ed25519_bip32`. `SigningKey::fromExtendedTextEnvelope()` reads either file, and
`SigningKey::fromExtended($kL, $kR, ?$publicKey)` takes the two secret halves directly, with the verification key
optional. An extended key signs, witnesses and yields a credential exactly as a seed key does, so `TransactionSigner`
takes either, and a transaction can carry both.

The verification key is always computed from kL. The one in the file is checked against it, and a file whose key
does not match is refused.

Signatures are byte-identical to the reference extended-key scheme, and so to what cardano-cli and cardano-signer
produce for the same key and message. The nonce is SHA-512(kR || M) reduced modulo the group order, as the
reference derives it. PHP's sodium extension has no Ed25519 multiplication that leaves a scalar unclamped, so the
nonce point comes from libsodium's ristretto255 base multiplication, and every operation on kL, kR and the nonce
runs inside libsodium. The package decodes the point's ristretto255 encoding to its four Edwards candidates in PHP,
and keeps the one libsodium places in the prime-order subgroup. That choice uses the published half of the signature
and nothing else, and S is computed once, for the chosen point.

This needs PHP built against libsodium 1.0.18 or newer. Without GMP, reading a key takes about 20 milliseconds, and
so does each signature, most of it one modular exponentiation. A seed key signs in well under one millisecond.
...
Extended keys are checked against the tools that write them. Two throwaway keys under `tests/fixtures/cardano-signer`,
never funded, sign each committed message to cardano-signer's signature byte for byte, and witness a mainnet body
exactly as cardano-cli does. The RFC 8032 seeds signed through the extended path reproduce the published signatures.
Signing with kL values no real key has, including values at or above 2^255, reproduces the signatures of an
independent Python signer under `tests/fixtures/ed25519-bip32`. The ristretto255 decoder is checked against the RFC
9496 vectors under `tests/fixtures/ristretto255`.

None of `tests` ships in the Composer package: `.gitattributes` leaves it out of the archive, so the throwaway keys
never reach a consumer's `vendor` directory.
