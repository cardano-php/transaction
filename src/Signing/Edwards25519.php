 ...
 * running time depends on the values involved, so it is safe only for data an observer already has. SigningKey hands
 * it exactly that: the nonce commitment R of a signature, which is published as the first half of the signature, and
 * a verification key, which is published in every witness. The secret scalars stay inside libsodium, which computes
 * the point and hands back its ristretto255 encoding; all this class does is turn that public encoding into the four
 * Edwards encodings it stands for.
 *
 ...
 * candidates() lists all four. Choosing between them is SigningKey's, which asks libsodium which one is in the
...
final class Edwards25519
{
     ...
     * carry; the other three differ from it by a point of order dividing four.
     *
